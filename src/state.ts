...
import { atomFamily, atomWithDefault, atomWithLazy, atomWithRefresh, atomWithReset, loadable } from 'jotai/utils';
import {
  Article,
  AvailableTimeSlots,
  Booking,
...
export const availableTimeSlotsState = atom<Promise<AvailableTimeSlots[]>>(mock7DaysTimeSlots);

export const articlesState = atom<Promise<Article[]>>(mockArticles);

export const departmentGroupsState = atom<Promise<DepartmentGroup[]>>(mockDepartmentGroups);
...
export const feedbackCategoriesState = atom<Promise<string[]>>(mockFeedbackCategories);
...
export const departmentHierarchyState = atom(async (get) => {
  const [groups, deps] = await Promise.all([get(departmentGroupsState), get(departmentsState)]);
  ...
  loadable(
    atom(async (get) => {
      ...
      return {
        ...
        departments: departments.filter((d) => toLowerCaseNonAccentVietnamese(d.name).includes(normalizedKeyword)),
        news: news.filter(
          (n) =>
