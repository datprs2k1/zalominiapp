...
export function ScheduleItem({ schedule }: ScheduleItemProps) {
  return (
    <TransitionLink to={`/schedule/${schedule.id}`} className="flex w-full flex-col gap-3 rounded-xl bg-white p-4">
      <div className="flex items-center justify-between gap-11 font-medium">
        <div className="text-base">{schedule.department.title}</div>
      ...
      <div className="flex items-center justify-end gap-1.5 pt-1 text-center text-xs">
        <div className="rounded-md bg-highlight px-2 py-1.5 text-primary">Xem chi tiết</div>
      </div>
    </TransitionLink>
  );
}
