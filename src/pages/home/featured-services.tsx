          ...
          {Array.from({ length: 2 }).map((_, index) => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black/70 via-black/20 to-transparent p-4">
                ...
                <div className="h-10 bg-emerald-300 rounded-xl w-full"></div>
              </div>
            </div>
          ))}
        </div>
          ...
          {Array.from({ length: 4 }).map((_, index) => (
            ...
            >
              ...
              <div className="flex flex-col flex-1 p-3 md:p-4">
                ...
                <div className="mt-auto h-8 bg-emerald-300 rounded-lg w-full"></div>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="flex gap-4 overflow-x-auto pb-2 px-1 hide-scrollbar scroll-snap-x md:hidden">
            {posts.map((post) => (
              <TransitionLink
                key={post.id}
                to={`/service/${post.id}`}
                className="flex-shrink-0 w-[60vw] h-[90vw] max-w-[340px] max-h-[510px] rounded-3xl shadow-md bg-white border border-gray-100 transition-all duration-300 group focus:outline-none focus:ring-2 focus:ring-emerald-500 cursor-pointer text-left active:scale-95 overflow-hidden relative scroll-snap-align"
              ...
              >
                <div className="relative w-full h-full">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-emerald-900/80 via-emerald-900/30 to-transparent opacity-70 group-hover:opacity-90 transition-opacity duration-300"></div>
                  ...
                  <div className="absolute bottom-0 left-0 w-full p-4 flex flex-col">
                    ...
                    />
                    <span className="flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-500 text-white text-base font-semibold active:scale-95 text-center shadow-lg group-hover:shadow-emerald-500/50 transition-all duration-300">
                      Xem chi tiết
          ...
          <div className="hidden md:grid md:grid-cols-3 lg:grid-cols-4 md:gap-5 md:overflow-x-visible md:pb-0 px-1">
            {posts.map((post) => (
              <TransitionLink
                key={post.id}
                to={`/service/${post.id}`}
                className="flex flex-col bg-white rounded-2xl shadow-sm border border-gray-100 transition-all duration-300 group focus:outline-none focus:ring-2 focus:ring-emerald-500 cursor-pointer text-left active:scale-95 overflow-hidden min-h-[220px] hover:shadow-md hover:-translate-y-1"
                aria-label={`Xem dịch vụ ${post.title.rendered}`}
              >
                <div className="relative w-full aspect-[4/3] bg-gray-100 overflow-hidden">
                  ...
                  <div className="absolute top-3 right-3">
                    <span className="bg-emerald-500/80 backdrop-blur-sm text-white text-xs font-medium px-2 py-1 rounded-md">
                      Nổi bật
                    </span>
                  </div>
                </div>
                <div className="flex flex-col flex-1 p-3 md:p-4">
                  <div
                    className="text-emerald-700 text-base font-bold mb-1 line-clamp-2 leading-tight group-hover:text-emerald-600 transition-colors"
                    dangerouslySetInnerHTML={{ __html: post.title.rendered }}
                  />
                  ...
                  <span className="mt-auto flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-gradient-to-r from-emerald-600 to-emerald-500 text-white text-sm font-semibold active:scale-95 text-center shadow-sm group-hover:shadow-md transition-all">
                    Xem chi tiết
